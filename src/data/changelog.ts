...
export const changelogItems: ChangelogItem[] = [
  {
    ...
    date: '2026-06-01',
    changes: [
      '打开书架后立即展示本地书籍，并在后台顺序预取置顶与最近阅读的云端进度，默认处理前 6 本，同步期间标题下方会显示当前进度',
      '修复首次启动时书架早于设置加载完成而跳过云端进度预取的问题，设置就绪后会自动补拉一次',
      'WebDAV 阅读进度下载加入 5 分钟缓存和同书并发复用，书架与阅读页共享路径拼接、远端解析和新旧判断，减少重复请求',
      '云端进度不存在时静默跳过；网络、HTTP 或 JSON 异常时继续展示本地数据，并短暂提示已回退到本地进度',
      'WebDAV 设置新增书架云端进度预取数量，可按需调整请求本数，设为 0 时关闭书架预取',
    ]
  },
  {
    version: '1.1.4',
